# Definition: Uses Pydantic models to enforce schema and type validation on the LLM’s output.
# How it works:
#   You define a Pydantic class with fields and types (e.g., str, int, List[str]).
#   LangChain’s with_structured_output() or PydanticOutputParser ensures the LLM output is parsed into that model.
#   If the output doesn’t match, Pydantic raises validation errors.
# Practical Guidance:
#   Use Pydantic structured output when building robust, production-ready pipelines where type safety and validation are critical (e.g., passing LLM outputs into APIs, databases, or downstream systems).
